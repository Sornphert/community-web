...
- **Context-aware back link on the global feeds** — Following (`app/(app)/following`) and the universal profile (`app/(app)/people/[id]`) live in the teacher-AGNOSTIC (app) shell, so their back link used to always go to `/home` (ejecting you from the community you came from). The sidebar Following link and the follower/following list rows (`follow-controls.tsx`) now pass `?from=<slug>` when inside a teacher shell; both pages resolve it to a real teacher (guarding a stale/bogus param) and return to `/t/<slug>/community` with the teacher's name, falling back to Home when absent. `sidebar.tsx` `isActive` strips the query string so the tab still highlights.
- **MT-shell error/loading boundaries** — the single-tenant convention ("every dynamic route ships loading/error/not-found") had drifted in the `/t/[slug]` shell. Added a group-level `error.tsx` + `loading.tsx` at `app/t/[slug]/(community)/` (catch-all for admin/events/members/messages/saved/profile/landings; per-route boundaries still override) and a root `app/error.tsx` as the ultimate net for `/home`, the `[slug]/layout` teacher fetch, and pre-auth pages. Boundary components read the slug via `useParams` (no params prop).
- **Channels drag-to-reorder** — `channels-manager.tsx` replaced the up/down arrow buttons with drag-and-drop (same pattern as the classroom `ClassroomAdminGrid`: optimistic local `order`, grip handle, between-row drop bar, vertical top/bottom-half detection), persisting via the existing `reorderChannels` action. Rows being edited/confirming-delete aren't draggable; dragging freezes while a save is in flight.
