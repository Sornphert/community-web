...
// "Everyone" (@all) option.
//
// The popover matches the display name as a case-insensitive prefix/substring
// over a spaceless query — good enough for a ~30-member community.
export function MentionTextarea({
  ...
  // The "@everyone" pseudo-option, offered to admins and filtered by query.
  const showAll =
    canMentionAll &&
    query !== null &&
  ...
  // Flat option list the keyboard navigates. `all` sentinel first when shown.
  ...
  // Recompute the active mention query from the caret position.
  ...
  // Insert the chosen token in place of the `@query`, then close the popover.
  ...
  return (
    <div className="relative">
      <textarea
        ...
        onBlur={() => {
          // Delay so a click on an option registers before the popover closes.
